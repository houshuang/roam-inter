export const btree = [
  [
    {
      title: "Zettelkasten",
      uid: "uSQWybZdh",
      children: [
        {
          string: "Invented by [[Niklas Luhmann]]",
          uid: "p6P9Qsj03",
          children: [
            {
              string: "Original writings",
              uid: "ptgRUguju",
              children: [
                {
                  string: "http://luhmann.surge.sh/learning-how-to-read",
                  uid: "AwuWuqCH8",
                  order: 0
                },
                {
                  string:
                    "http://luhmann.surge.sh/communicating-with-slip-boxes",
                  uid: "3kcG4hApv",
                  order: 1
                }
              ],
              order: 0
            },
            {
              string: "Videos",
              uid: "u2yPWfUDt",
              children: [
                {
                  string:
                    "Niklas Luhmann – 2016 – Ich denke ja nicht alles allein (Zettelkasten als Zweitgedächtnis)",
                  uid: "C0mGBxFuM",
                  children: [
                    {
                      string: "https://www.youtube.com/watch?v=NbncA7bDl70",
                      uid: "VOYEhFOTU",
                      order: 0
                    }
                  ],
                  order: 0
                }
              ],
              order: 1
            }
          ],
          order: 4
        },
        {
          string: "Secondary literature",
          uid: "AJvATnCfO",
          children: [
            {
              string: "Books",
              uid: "1-i32PkQL",
              children: [
                {
                  string: 'Book: "[[How to Take Smart Notes📒]]", Sonke Ahrens',
                  uid: "QD7eibwZI",
                  order: 0
                },
                {
                  string: "Die Zettelkasten methode: Kontrolliere dein Wissen",
                  uid: "mitubUycT",
                  children: [
                    {
                      string: "by [[Sascha Fast]] and [[Christian Tietze]]",
                      uid: "zNhmBD68W",
                      order: 0
                    },
                    {
                      string:
                        "Souveränität ist *die* Schlüsselkompetenz in unserer modernen Welt im Umgang mit Wissen und Informationen. Dabei sind Schnelligkeit, Sicherheit und Flexibilität die entscheidenden Komponenten, um diese Souveränität für sich zu gewinnen. Die Zettelkastenmethode ist eine flexible und individuell anpassbare Technik mit Wissen umzugehen. Sie ist auf jeden System mit ein paar Handgriffen realisierbar, ohne dass man komplizierte Programme verstehen muss. In ihrer Einfachheit ist sie unübertroffen und ist der (fast) unsichtbare Helfer für dich im Umgang mit dem Wissen. Niklas Luhmann, einer der produktivsten Wissenschaftler aller Zeiten, sagte einmal, er müsse gar nicht schreiben, das mache der Zettelkasten für ihn. Als ich das Prinzip hinter seinem Zettelkasten, zusammen mit Christian Tietze, in die moderne Welt der digitalen Technik übertrug, war ich überrascht, wie viel Wahrheit in Luhmanns Aussage steckt.",
                      uid: "UfRWDYkcg",
                      order: 1
                    }
                  ],
                  order: 1
                }
              ],
              order: 0
            },
            {
              string: "Blog posts",
              uid: "IjzJ6gW6u",
              children: [
                {
                  string:
                    "https://www.lesswrong.com/posts/NfdHG6oHBJ8Qxc26s/the-zettelkasten-method-1#Very_Short_Summary",
                  uid: "V_UyLkKYo",
                  order: 0
                }
              ],
              order: 1
            }
          ],
          order: 5
        },
        {
          string: "Related concepts",
          uid: "kiTjdRuXJ",
          children: [
            {
              string: "[[Evergreen Notes]] [[Note taking]] [[zettel]]",
              uid: "-mkGhgfmG",
              order: 0
            }
          ],
          order: 2
        },
        {
          string: "Process log",
          uid: "_fiud5Ish",
          children: [
            {
              string:
                "Sorting through on [[April 21st, 2020]] while recording a video. Spent about 20 minutes to process 17 linked references. Feels a bit slow, but I see some interesting structure emerging.",
              uid: "7YIL4-zRN",
              order: 0
            }
          ],
          order: 1
        },
        {
          string: "Questions",
          uid: "034EyoKfe",
          children: [{ string: "((Vniu79e6D))", uid: "XDNs0H-lN", order: 0 }],
          order: 3
        },
        {
          string: "Dangers / pitfalls",
          uid: "xAAnhWeED",
          children: [
            { string: "{{embed: ((S6tbdS_D5))}}", uid: "BfwML4qWt", order: 0 },
            { string: "((J5d_OHzNG))", uid: "zFoOedjn7", order: 1 },
            { string: "", uid: "BQKCRf7EF", order: 2 }
          ],
          order: 8
        },
        {
          string: "Interfaces",
          uid: "DBt-mayrY",
          children: [
            {
              string: "Hierarchical (like Roam)",
              uid: "diMjxDLtf",
              children: [
                { string: "((TbLkQUZg5))", uid: "Ay5G_1l5w", order: 0 }
              ],
              order: 0
            },
            {
              string: "Visual",
              uid: "kYDDJqY86",
              children: [
                {
                  string: "{{embed: ((thpshQegz))}}",
                  uid: "2ND86udzv",
                  order: 0
                }
              ],
              order: 1
            },
            { string: "((VgF2tpaGI))", uid: "L6Vq0tlJv", order: 2 }
          ],
          order: 9
        },
        {
          string: "Tools",
          uid: "YCCWs376T",
          children: [
            {
              string: "Apps",
              uid: "XloPWEY3P",
              children: [
                {
                  string: "[[TinderBox]] by [[Beck Tench]]",
                  uid: "UyVYKA61J",
                  children: [
                    {
                      string:
                        "https://www.becktench.com/blog/2018/11/12/using-zettelkasten-and-tinderbox-to-document-a-literature-review",
                      uid: "gsYg7l2-j",
                      order: 0
                    }
                  ],
                  order: 3
                },
                {
                  string: "[[Devonthink]] ",
                  uid: "If41Yn37A",
                  children: [
                    {
                      string:
                        "http://www.organizingcreativity.com/2013/09/luhmanns-zettelkasten-with-devonthink/",
                      uid: "p-pngoz2z",
                      order: 0
                    }
                  ],
                  order: 2
                },
                {
                  string: "[[Hugo]] (h/t [[Christian Tietze]])",
                  uid: "RKV-VOSA-",
                  children: [
                    {
                      string:
                        "https://bryanapperson.com/blog/creating-a-zettelkasten-with-hugo/",
                      uid: "LXaFj02dL",
                      order: 0
                    }
                  ],
                  order: 5
                },
                {
                  string:
                    "[[Vim]] (h/t [[Christian Tietze]]) based on [[vimwiki]]",
                  uid: "GyMuL0LBZ",
                  children: [
                    {
                      string: "https://www.bitchute.com/video/wlFZJ8ltQTgg/",
                      uid: "mU8KAPjxW",
                      order: 0
                    }
                  ],
                  order: 6
                },
                {
                  string:
                    "[[Notion]], moving from [[The Archive]] from [[Zettelkasten.de]] by [[Ralf Westphal]] (@ralfw)",
                  uid: "xwDAyYAqn",
                  children: [
                    {
                      string:
                        "https://forum.zettelkasten.de/discussion/404/moving-on#latest",
                      uid: "RsYgrvSZH",
                      order: 0
                    },
                    {
                      string:
                        "Highlights ease of formatting, including images, and viewing the notes in different layouts.",
                      uid: "DalBsFC6v",
                      children: [
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2Fhbq0xI5rP7?alt=media&token=0f8116ea-a8df-4cac-b67f-cd1f1b6cb98f)",
                          uid: "zXYGUL1Fh",
                          order: 2
                        },
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2F7bna6XPFI4?alt=media&token=f0b9b59e-376a-41cd-8da7-9402890d2462)",
                          uid: "5xev05JMW",
                          order: 1
                        },
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2Fj6sQoHP6OK?alt=media&token=42bf920e-3646-439b-85f7-7d6a67b0a686)",
                          uid: "M7QaRhcbK",
                          order: 0
                        }
                      ],
                      order: 4
                    },
                    {
                      string: "Different formats",
                      uid: "thpshQegz",
                      children: [
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2Fyn0A6NudQs?alt=media&token=28adeafc-753e-4941-a30e-4110dc8418a2)",
                          uid: "0tFKRvCsy",
                          order: 3
                        },
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2FixKcnFwehp?alt=media&token=deb2b19f-99bb-4f8d-9e83-c92fc012930b)",
                          uid: "YN2HksdUU",
                          order: 2
                        },
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2Fr6s-klNs8Y?alt=media&token=c2b0754f-0d2e-4b7b-8adb-0abaefbad8e4)",
                          uid: "efk8uysnu",
                          order: 1
                        },
                        {
                          string:
                            "![](https://firebasestorage.googleapis.com/v0/b/firescript-577a2.appspot.com/o/imgs%2Fapp%2Fstian%2FhCzgo-uVs0?alt=media&token=767f87ab-0ef7-43f4-a973-d86478588666)",
                          uid: "T34wHZIIp",
                          order: 0
                        }
                      ],
                      order: 3
                    },
                    {
                      string: "Twitter",
                      uid: "ucUbIo65W",
                      children: [
                        {
                          string:
                            "https://twitter.com/houshuang/status/1232239406452531200 #myTwitterThreads",
                          uid: "mCZFIs-kf",
                          order: 0
                        }
                      ],
                      order: 2
                    },
                    {
                      string:
                        "https://forum.zettelkasten.de/discussion/446/views-of-a-zk#latest",
                      uid: "Ekl41m2yK",
                      order: 1
                    }
                  ],
                  order: 4
                },
                {
                  string: "[[The Archive]] by [[André Chaperon]] ",
                  uid: "0PHa2Y_QR",
                  children: [
                    {
                      string:
                        "https://www.youtube.com/channel/UCtGCJRJqOVeJUkPkcdHwELw",
                      uid: "rIeBjh-KL",
                      order: 0
                    }
                  ],
                  order: 7
                },
                {
                  string: "Roam",
                  uid: "jnWl6f9b-",
                  children: [
                    {
                      string: "Roam as a [[Slip box]] from [[David K. Laing]]",
                      uid: "MCmLZFHmz",
                      children: [
                        {
                          string:
                            "https://davidklaing.com/blog/2020/02/01/roam-as-a-slip-box.html",
                          uid: "uWBgUJRi1",
                          order: 0
                        },
                        {
                          string: "Distinguishing two kinds of notes",
                          uid: "VgvMoXhOC",
                          children: [
                            {
                              string:
                                "literature notes (paraphrases of ideas and arguments encountered during reading)",
                              uid: "s6VIO999q",
                              order: 0
                            },
                            {
                              string:
                                'permanent notes ("original" thoughts/insights)',
                              uid: "fe14UtyfX",
                              order: 1
                            }
                          ],
                          order: 1
                        },
                        {
                          string:
                            "__Both types of notes are written in publishable-quality prose, and each note is meant to include enough detail that it could be read and understood on its own.__",
                          uid: "4KWtQcLeD",
                          order: 2
                        },
                        {
                          string: "Workflow",
                          uid: "AT8Ueeeet",
                          children: [
                            {
                              string:
                                "writes all his notes on [[Daily notes]], tags litnote/permnote (sometimes in the same paragraph).",
                              uid: "SLjAfUN5-",
                              order: 0
                            }
                          ],
                          order: 3
                        }
                      ],
                      order: 0
                    },
                    {
                      string:
                        "https://shime.sh/how-i-apply-zettelkasten-to-roam #Newsletter #Zettelkasten",
                      uid: "npFmNjg9f",
                      order: 1
                    },
                    {
                      string: "{{embed: ((bUDunX4bB))}}",
                      uid: "9bM7exu6M",
                      order: 2
                    }
                  ],
                  order: 0
                },
                {
                  string: "{{embed: ((ky6cQvIp9))}}",
                  uid: "4Hu1nWdes",
                  order: 1
                },
                {
                  string: "{{embed: ((AcE-X_aty))}}",
                  uid: "21FzmJeu2",
                  order: 8
                }
              ],
              order: 0
            },
            {
              string: "Web-based tools",
              uid: "NyNDowosR",
              children: [
                {
                  string:
                    "[[zettel.io]] some kind of public Zettelkasten with [[Twitter]] integration? ",
                  uid: "1RpCxyIql",
                  children: [
                    {
                      string: "https://zettel.io/kasten",
                      uid: "zUuGOGr6j",
                      order: 0
                    }
                  ],
                  order: 0
                }
              ],
              order: 1
            }
          ],
          order: 10
        },
        {
          string: "Theory",
          uid: "DtT1kSjpP",
          children: [
            {
              string:
                "Zettelkasten as a [[lattice]] of thoughts from [[Zettelkasten.de]] by [[Gerrit Scholle]]",
              uid: "rgtfFBpR-",
              children: [
                {
                  string: "https://zettelkasten.de/posts/lattice-of-thoughts/",
                  uid: "9BJ3TNyZH",
                  order: 0
                },
                {
                  string: "read [[February 27th, 2020]] [[March 31st, 2020]]",
                  uid: "XqqpTuofD",
                  order: 1
                }
              ],
              order: 2
            },
            {
              string: "[[Rich picture]]  from [[Ralf Westphal]]",
              uid: "Onl9J8n8I",
              children: [
                {
                  string: "https://en.wikipedia.org/wiki/Rich_picture",
                  uid: "3XnyvB-Ij",
                  order: 0
                }
              ],
              order: 1
            },
            {
              string: "[[Grunded theory]] from [[Ralf Westphal]] ",
              uid: "jD63KzC6E",
              children: [
                {
                  string: "http://www.groundedtheory.com/what-is-gt.aspx",
                  uid: "Varofqyi8",
                  order: 0
                }
              ],
              order: 0
            }
          ],
          order: 7
        },
        {
          string: "Workflow",
          uid: "xBtP_kiSw",
          children: [
            {
              string:
                "From [[Zettelkasten.de]] ([[discordian]]) on how to use Zettelkasten to create lectures",
              uid: "3cw8S71rp",
              children: [
                {
                  string:
                    "[[Gordon Pasks]] [[Conversation Theory]] Pask arranges teaching materials into __[[Entailment Meshes]]__, which are build out of __Topics__ (comparable to zettels) and arranged (mainly) into overlapping __Coherences__.",
                  uid: "_rPpMWr-7",
                  order: 5
                },
                {
                  string:
                    "The inspiration comes from close colleagues historian father, who had this system of writing his materials on perforated index cards and then using a steel rod to 'fish out' materials for a given lecture from a giant archive drawer. Basically, he had a mechanical tagging system and could prepare a lecture in a moment's notice with it.",
                  uid: "DFiaSSMYE",
                  order: 2
                },
                {
                  string:
                    "https://forum.zettelkasten.de/discussion/865/introduction-teaching-using-zettelkasten-and-paskian-entailment-meshes#latest",
                  uid: "F6DB0ceAD",
                  order: 0
                },
                {
                  string:
                    "The idea seems to boil down to this: if the student knows some of the topics, an explanation for the unknown topics is implied by the coherence.",
                  uid: "YPADikapb",
                  order: 4
                },
                {
                  string:
                    "A coherence is set of three or more topics with the curious property that any topic in a given coherence is entailed by the remaining topics. For example, the Topics {Function, Data, Transformation} form a coherence, since concepts of Function and Data imply a Transformation (of the data), Transformation and Data imply a Function (specifying the transformation) and Transformation and Function imply Data (to be transformed).",
                  uid: "EynmxtCfE",
                  order: 3
                },
                {
                  string:
                    "^^There is a rich discussion in this thread that I haven't processed, worth coming back to^^ Resurface [[March 26th, 2020]]",
                  uid: "qxmar4_10",
                  order: 1
                }
              ],
              order: 0
            },
            { string: "((hCcOo7G1y))", uid: "an4fBArhK", order: 1 }
          ],
          order: 11
        },
        {
          string: "Videos of workflows",
          uid: "CKdixajDr",
          children: [
            {
              string:
                "[[[[Zettelkasten]]-p]] #video by [[Shu Omi]] #roam-workflow",
              uid: "bUDunX4bB",
              children: [
                {
                  string: "https://www.youtube.com/watch?v=rOSZOCoqOo8",
                  uid: "UaRtRG3d8",
                  order: 0
                }
              ],
              order: 1
            },
            { string: "{{embed: ((0PHa2Y_QR))}}", uid: "eMyMtVgUJ", order: 0 }
          ],
          order: 12
        },
        {
          string: "Places to discuss",
          uid: "DWQkjaYy-",
          children: [
            { string: "[[Zettelkasten.de]]", uid: "gbOaF6kza", order: 0 },
            {
              string:
                "[[TakingNoteNow]] blog, lots of great insights around [[Note taking]] and Zettelkasten, also from many earlier than [[Niklas Luhmann]]",
              uid: "0NwBghnAM",
              children: [
                {
                  string: "http://takingnotenow.blogspot.com/",
                  uid: "ZjmORXbeE",
                  order: 0
                }
              ],
              order: 1
            }
          ],
          order: 6
        },
        {
          string: "People/connections",
          uid: "uhYFSh56s",
          children: [
            {
              string: "[[Christian Wahl]]",
              uid: "M6ZcbU5po",
              children: [
                { string: "((FWxjcMwu2))", uid: "D3Q-tb9Xe", order: 0 }
              ],
              order: 0
            }
          ],
          order: 13
        },
        { string: "#public", uid: "tRARBNFl4", order: 0 }
      ]
    }
  ]
];

export const btreeBlock = [
  [
    {
      string: "Start",
      uid: "rz7b2mX1Fx",
      order: 0
    },
    {
      string: "[[Log0]]",
      uid: "z7b2mX1Fx",
      order: 1
    },
    {
      string: "[[Log]]",
      uid: "b7b2mX1Fx",
      order: 2,
      children: [
        {
          string: "[[Roam Inter]] 05:56",
          uid: "h10M8PQpL",
          order: 0,
          children: [
            {
              string:
                "had an insight last night - instead of everyone writing in the same place, what if each person has their own space, but they just block-ref each other? There might also be a special mode for chatting... Would solve a lot of problems, and be congruent with the way I collaborated with Conor...",
              uid: "d4Zlg5YKN",
              order: 0
            },
            {
              string:
                "I think the first step is to diff two JSON structures, and calculate the operations needed to change one to the other.",
              uid: "TapwdDCN6",
              order: 1
            },
            {
              string: "Then try to do a simple one way sync",
              uid: "tady6lx6S",
              order: 2
            },
            {
              string:
                "Also need button/tag, and MutationObserver or some other mechanism",
              uid: "nrW1eki95",
              order: 3
            },
            {
              string:
                "How do we deal with many ShareDB changes? Can we replay them all?",
              uid: "Pjgon1MDr",
              order: 4
            }
          ]
        }
      ]
    },
    {
      string: "lastLog",
      uid: "z732m41Fx",
      order: 3
    }
  ]
];

export const btreeBlock2 = [
  [
    {
      string: "Start",
      uid: "rz7b2mX1Fx",
      order: 0,
      children: [
        {
          string: "[[Log]]",
          uid: "b7b2mX1Fx",
          order: 1,
          children: [
            {
              string: "[[Roam Inter]] 05:56",
              uid: "h10M8PQpL",
              order: 0,
              children: [
                {
                  string:
                    "had an insight last night - instead of everyone writing in the same place, what if each person has their own space, but they just block-ref each other? There might also be a special mode for chatting... Would solve a lot of problems, and be congruent with the way I collaborated with Conor...",
                  uid: "d4Zlg5YKN",
                  order: 0
                },
                {
                  string:
                    "I think the first step is to diff two JSON structures, and calculate the operations needed to change one to the other.",
                  uid: "TapwdDCN6",
                  order: 1
                },
                {
                  string: "Then try to do a simple one way sync",
                  uid: "tady6lx6S",
                  order: 2
                },
                {
                  string:
                    "How do we deal with many ShareDB changes? Can we replay them all?",
                  uid: "Pjgon1MDr",
                  order: 5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      string: "[[Log2]]",
      uid: "b7b2mX1Fy",
      order: 2,
      children: [
        {
          string: "Start-deep",
          uid: "11rz7b2mX1Fx",
          order: 0
        },
        {
          string:
            "Also need button/tag, and MutationObserver or some other mechanism",
          uid: "nrW1eki95",
          order: 3
        },
        {
          string: "lastLog",
          uid: "z732m41Fx",
          order: 4
        }
      ]
    }
  ]
];
